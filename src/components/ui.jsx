...
export function InsulRec({section, preR, addR}) {
  ...
  // calcRtoAdd inline to avoid circular dep
  let suggested = null;
  if (pre <= rule.threshold || section === "kneeWall" || section === "extWall1" || section === "extWall2" || section === "fnd") {
    if (section === "kneeWall" && pre >= 20) suggested = null;
    else if ((section === "extWall1" || section === "extWall2") && pre > 0) suggested = null;
    else if (section === "fnd" && pre >= 10) suggested = null;
    else suggested = Math.max(0, rule.target - pre);
  }
...
export function CK({checked,onChange,label,color,strike,small}) {
  return <label style={{...S.ck,fontSize:small?10:12,...(color?{color}:{}),cursor:"pointer",...(strike?{textDecoration:"line-through"}:{})}}><input type="checkbox" checked={!!checked} onChange={e=>onChange(e.target.checked)} style={{marginRight:6,accentColor:"#2563EB",width:small?14:16,height:small?14:16,flexShrink:0}}/><span style={{lineHeight:1.3}}>{label}</span></label>;
}

export function BtnGrp({value,onChange,opts}) {
  return <div style={{display:"flex",gap:3}}>{opts.map(o=><button key={o.v} type="button" onClick={()=>onChange(value===o.v?"":o.v)} style={{padding:"7px 10px",borderRadius:6,border:value===o.v?`2px solid ${o.c}`:"1px solid rgba(255,255,255,.1)",background:value===o.v?`${o.c}22`:"rgba(255,255,255,.03)",color:value===o.v?o.c:"#64748b",fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",minWidth:40,minHeight:36}}>{o.l}</button>)}</div>;
}
...
export function SI({l,v,c}) {
  return <div style={S.si}><span style={{fontSize:9,color:"#64748b",textTransform:"uppercase",letterSpacing:".04em"}}>{l}</span><span style={{fontSize:13,fontWeight:600,color:c||"#e2e8f0",marginTop:2}}>{v}</span></div>;
}
...
export function UserMgmt({users, onSave, onDelete, onClose}) {
  ...
  const roleBadge = (r) => {const m=ROLES.find(x=>x.key===r); return <span style={{fontSize:9,padding:"2px 6px",borderRadius:4,background:"rgba(37,99,235,.15)",color:"#93C5FD"}}>{m?.icon} {m?.label||r}</span>;};
